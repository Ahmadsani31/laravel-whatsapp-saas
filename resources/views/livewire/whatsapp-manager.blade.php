<div class="space-y-6">
    ...
    <div>
        ...
        <div class="space-y-3">
            ...
            <div class="flex justify-between items-center">
                <div class="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <span class="ml-2 px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded text-xs font-medium">
                        {{ count(array_filter(array_map('trim', preg_split('/[\r\n,;]+/', $numbers)))) }} numbers detected
                    </span>
                    @endif
                </div>

                <button wire:click="checkBulkNumbers"
                    wire:loading.attr="disabled"
            ...
            <div wire:loading wire:target="checkBulkNumbers" class="mt-4 p-4 bg-blue-50 dark:bg-blue-900 rounded-lg border border-blue-200 dark:border-blue-700">
    ...
    <div class="p-4 rounded-lg border-l-4 {{ isset($checkResult['exists']) && $checkResult['exists'] ? 'border-green-500 bg-green-50 dark:bg-green-900' : 'border-red-500 bg-red-50 dark:bg-red-900' }}">
        <div class="flex items-center">
            <i class="fas {{ isset($checkResult['exists']) && $checkResult['exists'] ? 'fa-check-circle text-green-600 dark:text-green-400' : 'fa-times-circle text-red-600 dark:text-red-400' }} mr-2"></i>
            <span class="{{ isset($checkResult['exists']) && $checkResult['exists'] ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200' }}">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        ...
                        Valid: <span class="text-green-600 dark:text-green-400 font-medium">{{ count(array_filter($bulkCheckResults, fn($r) => $r['exists'] ?? false)) }}</span> |
                        Invalid: <span class="text-red-600 dark:text-red-400 font-medium">{{ count(array_filter($bulkCheckResults, fn($r) => !($r['exists'] ?? false))) }}</span>
                    </p>
                </div>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <span class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full {{ $result['exists'] ? 'bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200' : 'bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200' }}">
                                <i class="fas {{ $result['exists'] ? 'fa-check-circle' : 'fa-times-circle' }} mr-1"></i>
                                {{ $result['exists'] ? 'Valid' : 'Invalid' }}
                            </span>
                            @else
                            <span class="inline-flex items-center px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200">
    ...
    <!-- Send Result -->
    @if($sendResult)
    <div class="p-4 rounded-lg border-l-4 {{ isset($sendResult['success']) && $sendResult['success'] ? 'border-green-500 bg-green-50 dark:bg-green-900' : 'border-red-500 bg-red-50 dark:bg-red-900' }}">
        <div class="flex items-center">
            <i class="fas {{ isset($sendResult['success']) && $sendResult['success'] ? 'fa-check-circle text-green-600 dark:text-green-400' : 'fa-times-circle text-red-600 dark:text-red-400' }} mr-2"></i>
            <span class="{{ isset($sendResult['success']) && $sendResult['success'] ? 'text-green-800 dark:text-green-200' : 'text-red-800 dark:text-red-200' }}">
